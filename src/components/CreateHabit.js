...
function CreateHabit({formDisplayHidden, setFormDisplayHidden, listHabits}) {
    ...
    <FormCreateHabit onSubmit={createHabit}>
        <input type="text" placeholder="nome do hábito" value={habit.name} onChange={(e)=> setHabit({...habit, name: e.target.value})}></input>
        <DaysContainer>
        ...
        <div className="CreatHabit__footer">
            ...
            <button type="submit">{loading ? (<ThreeDots color="#fff" height={11} />) : ("Salvar") }</button>
        </div>
    </FormCreateHabit>
    ) : <></>
}
